...
def _get_agent_instruction() -> str:
    ...
    return (
        ...
        "5. **IMPORTANT**: Use only ASCII characters, numbers, hyphens, underscores, and dots in filenames. Replace spaces with hyphens or underscores. Avoid special characters like éñü, brackets [], parentheses (), or symbols that might cause path resolution issues. "
        "6. Examples of GOOD filenames: 'Machine-Learning-Paper-2024.pdf', 'Research_Report_Smith.pdf', 'Technical-Documentation.pdf' "
        "7. Examples of BAD filenames: 'Résumé (François).pdf', 'Paper[中文].pdf', 'Doc #1 & Notes.pdf' "
        ...
        "When you want to show an image to the user, use Markdown image syntax. The path must start from the 'data' directory. For example: '![alt text](data/document/thumbnail/image.jpg)'. **IMPORTANT**: Ensure image filenames use only ASCII characters - no special characters, spaces, or non-Latin characters that might break path resolution. Use hyphens or underscores instead of spaces. "
